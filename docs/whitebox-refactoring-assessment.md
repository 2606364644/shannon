...
**v2**：纠正 v1 的"确定性引擎替代 LLM"框架——真实架构是"加法层"（原 LLM 流水线照跑），补 5 个 v1 漏掉的问题（入口裁定橡皮图章、调用图静默残缺、tiered 审计未接线、Go/Java/PHP 零传播、GitNexus FULL 未实现）。
...
1. **recon.txt 删除**（`<` 仅原始）：整段 `### 4.1 Shared Controller Route Groups`（含 9 条格式规则 + 示例表 + pre-auth ⚠️ 警告）+ 整段 `## 4.2 Endpoint Security Context`（Method/Path/Auth/Middleware/Framework Origin/Ownership 表 + finale-rest/epilogue 框架检测 + Parameter Completeness 交叉验证）+ Route Mapper 的"分组检测、每路由独立行"指令 + Input Validator 的"枚举所有 input type 字段(Zod/Joi/Pydantic)、报告 wildcard 字段"指令 + `@include(shared/_endpoint-security-context.txt)`。
2. **连锁删除**（因 recon 不再产 4.1/4.2）：vuln-injection/xss/ssrf/authz 全删 `@include(_cross-route-enumeration.txt)` + conclusion-trigger 的 Cross-Route Verification 门控；vuln-xss 删 Endpoint Security Context starting_context；**vuln-authz 删整段 `### 0) Read Endpoint Security Context (REQUIRED)` + Framework Endpoint Guidance（finale-rest/epilogue IDOR 检测）——无替代，授权/IDOR 最大回退**。
3. **pre-recon-code.txt 删除**：Entry Point Mapper 的"identify API schema files (OpenAPI/Swagger/GraphQL)... Distinguish public vs auth-required"指令；Sink Hunter 的**强制两步模板流程**（glob 枚举 + 逐文件转义模式区分）+ **Cross-Variant Verification** + **Template Coverage Audit 完整性表**——模板 sink 检测在确定性层和 LLM 层**两头落空**。
4. **pre-recon-code.txt 新增（但不可执行）**：`Phase 0: Entry Point Adjudication` 指令（读 code_index → 置信度裁定 confirmed/rejected/reclassified → `save-deliverable --type ENTRY_POINTS`）——**依赖的 `save-deliverable` 工具代码里不存在**（§2.3），prompt 描述精细裁定、实际流水线全量 CONFIRM 覆盖。
5. **vuln prompt 一致新增**：`authentication_required`/`accessible_routes`/Source Completeness Rule（源粒度更强，但 `accessible_routes` 是自由填写、无 4.1 表 backing）。
...
## 0. 真实范式：确定性预分析层 + 原样保留的 LLM 流水线

### 0.1 真实流水线编排（`packages/whitebox/.../pipeline/workflows.py`）

```
run_preflight → run_credential_check → run_auth_validation
→ run_code_index          # 确定性：AST sink/call_graph/propagation
→ run_agent(PRE_RECON)    # LLM：照跑（但 prompt 已被改写，见 §0.3/§2.4）
→ run_save_adjudication   # 确定性：橡皮图章裁定（见 §2.3）
→ run_rebuild_call_chains # 确定性：BFS 重建链
→ run_agent(RECON)        # LLM：照跑（但 prompt 删了 4.1/4.2，见 §0.3）
...
### 0.3 v4 根因：能力替换，不是纯加法（recon 4.1/4.2 删除的连锁）
...
**重构**：`DEFAULT_RULES`（**实测 47 条** `SinkRule`，`sink_detector.py:67-215`；v1/v2 误称"197+"）。✅ 精确、可维护。但覆盖广度见 §1.6。

### 1.3 Slot 类型系统

基本对齐。重构把 `CMD-part-of-string`/`FILE-include` 并入 `CMD_ARGUMENT`/`FILE_PATH`，新增 `URL`/`GENERIC`。**平手。**

### 1.4 模板/视图文件 Sink 检测 ⚠️ 双层落空（v4 强化）
...
- **LLM 层（v4）**：pre-recon prompt 的"强制两步模板流程 + 变体审计 + Coverage Audit 表"**整段删除**，只剩一句泛泛"Find all dangerous sinks..."（diff 证据见修订说明 #3）。

**评估**：模板重前端项目，模板类 SSTI/XSS sink 在**确定性层和 LLM 层两头都没有专门覆盖**。❌❌

### 1.5 变体审计 ⚠️ 缺失（v4：prompt 层也删）

**原始**：pre-recon 强制变体覆盖审计（`pre-recon-code.txt:135-136`）。

**重构**：确定性无；**LLM 指令也删了**（v4）。❌
...
**重构**：`entry_points.py:detect_entry_points()` 按 language AST 模式匹配，confidence 0.30-0.95（硬编码），`needs_llm_review`。覆盖 ≥8 框架/类型（多于 v2 所说"6"）。✅ 检测确定性优于原始。

### 2.2 功能完整性对比（v4：schema/auth 指令在 prompt 层也删了）

| 功能 | 原始 | 重构 | 评估 |
|---|---|---|---|
| HTTP 路由检测 | ✅ LLM | ✅ AST 模式匹配 | **重构更可靠** |
| API Schema 优先读取 | ✅（OpenAPI/Swagger/GraphQL） | ❌ 代码无；**LLM 指令也删了**（v4） | **原始胜** |
| 认证标注 | ✅ 每入口标 public/需认证 | ❌ 模型无字段；**LLM 指令也删了**（v4） | **原始胜** |
...
### 2.3 入口裁定：橡皮图章 + Phase 0 不可执行（v3/v4）⚠️

**重构 `save_adjudication`**（`__init__.py:258-267`）无条件全置 `verdict=CONFIRMED, source=CODE_INDEX`，不看 confidence/`needs_llm_review`。

**比 v2 更糟**：
1. pre-recon prompt 有 `Phase 0: Entry Point Adjudication` 指令（v4 新发现）——要求按 confidence 裁定 confirmed/rejected/reclassified，写 `entry_points.json`。**但依赖的 `save-deliverable --type ENTRY_POINTS` 工具在 Python 代码库不存在**（只在 prompt 文本里）。**Phase 0 物理不可执行。**
...
### 2.4 两层架构（v4：recon 4.1/4.2 生产者被删）
...
**重构**：⚠️ **recon.txt 把 4.1/4.2 整段删除**（v4 diff 证据）。无"共享 handler 分组"、无"中间件链"、无"框架来源"细化，也无 `_endpoint-security-context.txt`/`_cross-route-enumeration.txt`（后者整个文件删）。这正是 §0.3 连锁的根。❌
...
**v3 写法 bug**：`analyze_intra` LHS 正则要求纯标识符，导致 `d["k"]=x`/`a,b=x,y`/`self.x=x`/`lst.append(x)` **全部静默漏检**。"容器过近似"只对读成立，对写是 false-negative。

### 3.3 分层审计：算了但没接（v3 确认）

`AUDIT_TIER1` agent + `audit-tier1.txt` 注册但全仓无调用；workflow 按漏洞类跑不按 chain/tier；`audit_input_builder` 按链构造器无生产调用方；plan-c Task 10 标 `[x]` 但未替换 dispatch。v2"未接线"论断正确。

### 3.4 漏洞研判 prompt：跨 prompt 一致的"删 4.1/4.2 依赖 + 加源粒度"（v4 重写）

v3 只 diff 了 vuln-injection。v4 diff 了 vuln-xss/ssrf/authz，发现**高度一致的跨 prompt 模式**：

**一致删除（三个 prompt 全删）**：`@include(_cross-route-enumeration.txt)` + conclusion-trigger 的 Cross-Route Verification 门控。
...
**vuln-injection 特有删除**：Branch Path Exhaustion（分支穷举）+ cross-route-enumeration（v3）。
...
| 模板文件 sink 覆盖 | ★★★★ | ☆ | **原始** | v4：双层落空 |
| 变体审计 | ★★★★ | ☆ | **原始** | v4：prompt 层也删 |
| 入口点确定性+置信度 | ★★ | ★★★★ | **重构** | 硬编码 |
| 入口 schema 优先 | ★★★★★ | ☆ | **原始** | v4：指令也删 |
| 入口认证标注 | ★★★★ | ☆ | **原始** | v4：指令也删 |
| 入口裁定把关 | ★★★ | ★ | **原始** | Phase 0 不可执行 |
| recon 4.1/4.2 结构化索引 | ★★★★ | ☆ | **原始** | v4：生产者删除 |
| 调用链确定性 | ★ | ★★★ | **重构（有限）** | 静默残缺 |
...
| **vuln-authz 框架 IDOR 检测** | ★★★★ | ☆ | **原始** | v4：最大回退、无替代 |
...
| # | 缺口 | 严重度 | 来源 | 备注 |
|---|---|---|---|---|
| 1 | 模板/视图文件 sink 检测 | **高** | v2 | v4：双层落空 |
| 2 | API schema 优先读取 | 中 | v2 | v4：指令也删 |
| 3 | 入口点认证标注 | 中 | v2 | v4：指令也删 |
| 4 | 网络可达性系统性过滤 | 低 | v2 | — |
| 5 | 变体审计 | 低 | v2 | v4：prompt 层也删 |
| 6 | Go/Java/PHP 传播支持 | **中-高** | v2 | 三语言零增益 |
| 7 | 入口裁定橡皮图章/Phase 0 不可执行 | 中 | v2/v3 | save-deliverable 缺失 |
| 8 | 调用图静默残缺 | **高** | v2 | 名匹配丢30-50% |
| 9 | 分层按链审计未接线 | 中 | v2/v3 | — |
| 10 | GitNexus FULL 未实现 | 中 | v2 | — |
| 11 | SSRF 覆盖仅 ~3/13 子类 | **高** | v3 | — |
| 12 | 路径穿越读取类 sink 盲区 | **高** | v3 | fopen/readFile 全缺 |
| 13 | XXE 完全缺失 | 中-高 | v3 | — |
| 14 | XSS 仅 ~1.5/5 上下文 | 中 | v3 | — |
| 15 | vuln prompt 删 Branch Path Exhaustion | 中 | v3 | injection |
| 16 | 传播写容器/属性漏检 | 中 | v3 | false-negative |
| 17 | sink 规则数文档错误 | 低 | v3 | 197+→47 |
| **18** | **recon 4.1/4.2 结构化索引删除** | **高** | v4 | 根因：路由分组/端点安全上下文/参数完整性全删 |
| **19** | **vuln-authz 丢框架 IDOR 检测** | **高** | v4 | Section 0 + Framework Guidance 删，无替代 |
| **20** | **pre-recon 模板 sink 方法论删除** | **高** | v4 | 强制两步流程+变体审计+Coverage Audit 删，确定性层也不覆盖 |
| **21** | **Phase 0 入口裁定指令不可执行** | 中 | v4 | 依赖不存在的 save-deliverable |
| — | LLM 仍是研判主导 | — | v2 | 修正 v1 误述 |
...
## 附录 A：关键文件对照（v4 增补 prompt diff 证据）
