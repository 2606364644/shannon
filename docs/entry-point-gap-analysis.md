...
> - EP-10 多源融合激活：fusion 步骤插入 pipeline（PRE_RECON → fusion → adjudication），扩展 LLM 4th source
...
| 多源融合 | ❌ 单源（LLM） | ✅ `entry_point_fusion.py` 四源去重（GitNexus > Schema > Framework > LLM），**pipeline 已激活**（PRE_RECON → fusion → adjudication） | **重构更优且已激活** |
...
| EP-3 | Webhook/Upload 端点检测 | prompt 覆盖 | ⚠️ LLM 层已恢复（Entry Point Mapper 升级 + fusion 有 webhook pattern）；确定性层无规则 | ~~低~~→LLM层已恢复 |
...
| ~~EP-10~~ | ~~多源融合 (已编码未激活)~~ | — | ✅ **已修复**：四源融合已激活（GitNexus > Schema > Framework > LLM） | ~~中~~→✅ 已修复 |
...
| EP+7 | 确定性 + LLM 入口融合 | 无 | 四源去重 + 自动化裁定流水线 | 重构新增 ✨ |
...
| 入口点融合（四源去重 + LLM 解析） | `packages/core/src/shannon_core/code_index/entry_point_fusion.py` |
