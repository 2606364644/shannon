...
> 本目录是 supernova-py（Python 重构版，分支 `feat/fork-py`）相对 `/root/shannon`（TypeScript 原始版，下称 **TS 版**）的安全扫描对照文档。三份分工，按需取用：
...
2. 想看某个优化「踩了什么坑、怎么排查」→ 对应章节进 [叙事配套](./refactor-scan-optimization-vs-ts.md)。
3. 关心二阶存储污点细节 → [机制深挖](./second-order-storage-taint-mechanism.md)。
4. 架构不变量（双轨铁律、确定性产物不喂 LLM 轨）→ [`../../CLAUDE.md`](../../CLAUDE.md) §1。
...
- 三份均为「测试绿 + 真机待跑」状态，**不把「单测绿」当「真机已验」**；真机硬证据（authz 0→21、hr 4→0 丢弃修复、11 个 curl PoC、exploit verdict 9 全 accepted）以主文档标注为准。
