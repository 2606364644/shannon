...
- 左侧**目录侧栏**（约 232px，sticky 吸顶、自身可滚动 `max-height:100vh`——多漏洞时目录内滚不撑页面）：
  - 分组镜像页面三区：**数据流树 (N)** / **认证·授权风险 (N)** / **排查过的入口 (N)**；
...
标题精炼为「数据流树」，组织方式放说明段（「每个危险点（sink）一棵树：红色流动线=打通（漏洞）；绿色线被 ✂ 剪断=防护拦下（安全）」）。每 sink 一卡，卡内 = **SVG 剪枝树**（水平汇聚：source 左列 → 向右汇聚到 sink 靶心）+ 枝条明细列表，两层数据联动。
...
- 树布局：枝条纵向堆叠、节点水平排布（source 左 → sink 右），共用函数按枝重复绘制（每枝自包含，研判看单枝不跳线），尺寸由折叠 + 限高 + 缩放治理。
...
| 前端 vitest+msw | 剪枝树渲染（打通流动线/剪断残端/黄绿盾/两种靶心）、链级⇄节点级标签文案、目录 scrollspy 与点击定位、剪断枝折叠、图区缩放平移、节点 tooltip、代码展开与 LLM 无码降级、认证/授权关卡链、排查过的入口区、筛选、空态、VulnCard 跳转、白话文案 zh/en 快照 |
