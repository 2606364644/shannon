...
标题精炼为「数据流树」，组织方式放说明段（「每个危险点（sink）一棵树：红色流动线=打通（漏洞）；绿色线被 ✂ 剪断=防护拦下（安全）；节点竖向对齐，绿线断得越靠右说明输入走得越远才被拦下」）。每 sink 一卡，卡内 = **SVG 剪枝树**（水平汇聚：source 左列 → 向右汇聚到 sink 靶心）+ 枝条明细列表，两层数据联动。
