...
Codex app-server 子进程（SDK 自带 pinned CLI runtime）
    ↓ model_provider = "supernova" (wire_api="chat")
GLM OpenAI 兼容端点
```
...
CodexProvider.call(prompt, cwd, model_tier, output_format, collector, ...)
 ...
 2. _build_invocation():
    ...
    · options.config = {model_provider: "supernova",
                        model_providers: {supernova: {name, base_url,
                                                      env_key: "SUPERNOVA_CODEX_API_KEY",
                                                      wire_api: "chat"}}}   ← 编程注入，不写 config.toml
    · options.env = 最小 env（PATH/HOME/TMPDIR + env_key 凭据变量 +
      proxy_url 非空时 HTTPS_PROXY/HTTP_PROXY/NO_PROXY=loopback）
      —— SDK env 是整体替换不是合并，必须显式构造
...
- `_build_invocation`：config 编程注入正确、env 整体替换语义、CODEX_HOME 每次 call 独立
...
1. **Phase 1 spike**：Python SDK API 面确认（`run_streamed` / thread resume / config 注入的 Python 对应物）+ GLM `wire_api="chat"` 最小跑通 + 子代理委派实测（探针雏形；不通则回炉设计）
...
- `packages/processor/src/agents/codex-sdk.ts`：`runStreamed` 事件流消费、config 编程注入（`model_providers` + `wire_api`）、CODEX_HOME per-invocation tempdir、env 整体替换 + allowlist、静默失败检测（0 output token）+ stderr wrapper、`chooseFinalText`（挑含 json 围栏的最后一条 agent_message）、thread 内 JSON 修复循环、usage 映射（input 含 cached 要减）
- `packages/processor/src/agents/claude-agent-sdk.ts`：`query()` 事件流 + env allowlist 对照

### 已知风险（spike 验证点）

1. **GLM `wire_api="chat"` 功能面**：Codex 主推 Responses API，chat 模式下部分特性（reasoning summary 等）可能降级；deepsec 网关走 `responses`，GLM 只有 chat completions。探针实测子代理委派是否正常。
2. **worker Docker 内 codex runtime**：wheel 为 py3-none-any（76KB），runtime 二进制获取/安装方式待确认；worker 是 linux/amd64（ARM64 有 tree-sitter/gitnexus 前科）。⚠️ 生效须 rebuild worker。
