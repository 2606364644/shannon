...
- [x] **Step 3: 实现 — 新增模型到 `parameter_models.py`**
...
- [x] **Step 1: 写失败测试 — CodeIndex 默认空 sink_call_sites**
...
- [x] **Step 3: 实现 — 增加 sink_call_sites 字段**
...
- [x] **Step 1: 写失败测试 — BaseParser 新方法存在且为抽象**
...
- [x] **Step 4: 运行测试 — 旧测试会失败（5 个 parser 都缺新方法）**
...
- [x] **Step 1: 写失败测试 — Python call 节点提取**
...
- [x] **Step 3: 实现 — PythonParser 新增三方法**
...
- [x] **Step 3: 实现 — TypeScriptParser 新增三方法**
...
- [x] **Step 3: 实现 — 创建 sink_detector.py 与规则库**
...
- [x] **Step 1: 写失败测试 — is_entry_hint + detect_sinks 主流程**
...
- [x] **Step 3: 实现 — 添加 detect_sinks + is_entry_hint + 内部辅助函数**
...
- [x] **Step 1: 写失败测试 — build_code_index 产出 sink_call_sites**
...
- [x] **Step 3: 实现 — 在 build_code_index 中调用 detect_sinks**
...
- [x] **Step 5: 跑完整 code_index 测试套件确认不破坏**
...
- [x] **Step 1: 写失败测试 — chain 上中间节点的 sink 也算分**
...
- [x] **Step 3: 实现 — 升级 ChainRiskScore.score**
...
- [x] **Step 5: 跑下游 tiered_audit 测试确认不破坏**
...
- [x] **Step 3: 标注 classify_sink 为 deprecated（不删除）**
...
- [x] `packages/core/src/shannon_core/code_index/parameter_models.py` 包含 `SinkCallSite`、`DangerousSlot`、`SlotContext`、`SinkCategory` 四个新模型；`SinkType` 仍存在。
- [x] `CodeIndex.sink_call_sites` 字段存在，pydantic 自动序列化进 `code_index.json`。
- [x] `BaseParser` 新增三个抽象方法 `iter_calls`/`destructure_call`/`extract_arg_expressions`，五种语言 parser 均实现。
- [x] `sink_detector.detect_sinks` 能在给定 FuncBlock 列表 + parser + source provider 的情况下，按规则库产出精确的 SinkCallSite 列表。
- [x] 规则库覆盖 5 语言 × SQL/Command/Deserialization/SSRF/XSS/SSTI/File/Redirect 共约 50 条规则，`needs_review_default=True` 正确应用在动态/模板类 sink。
- [x] `build_code_index` 集成 sink_detector，`code_index.json` 含 `sink_call_sites` 字段。
- [x] `risk_scorer.ChainRiskScore.score` 接受可选 `sink_call_sites` 参数；有则取 chain-wide max danger，无则降级回退到 `classify_sink`。
- [x] 端到端集成测试通过：多语言 repo、误报用例（commented sink / 变量名 sink）、`code_index.json` 序列化格式、id 格式 `"{file}:{caller_func}:{callee}:{line}:{col}"`。
- [x] 全量 `pytest packages/core/tests/code_index/` 通过，无回归。
- [x] `classify_sink` 标注为 DEPRECATED 但函数签名不变（兼容）。
