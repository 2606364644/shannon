...
# supernova-user can run whitebox scans on a fresh machine.
# Spec: docs/superpowers/specs/2026-07-13-supernova-user-provision-design.md
#
...
SUPERNOVA_PYPI_INDEX="${SUPERNOVA_PYPI_INDEX:-https://mirrors.aliyun.com/pypi/simple/}"
SUPERNOVA_NPM_REGISTRY="${SUPERNOVA_NPM_REGISTRY:-https://registry.npmmirror.com}"
SUPERNOVA_GITNEXUS_VERSION="${SUPERNOVA_GITNEXUS_VERSION:-1.6.8}"
SUPERNOVA_SKIP_DOCKER="${SUPERNOVA_SKIP_DOCKER:-0}"
...
install_docker() {
    if [ "$SUPERNOVA_SKIP_DOCKER" = "1" ]; then _skip "docker (SUPERNOVA_SKIP_DOCKER=1)"; return; fi
...
# 用户 + 系统级 uv + safe.directory（委托 ensure-supernova-user.sh）
ensure_user_and_uv() {
    local script
    script="$(dirname "${BASH_SOURCE[0]}")/ensure-supernova-user.sh"
    [ -f "$script" ] || _die "未找到 $script"
    SUPERNOVA_USER="$SUPERNOVA_USER" SUPERNOVA_HOME="$SUPERNOVA_HOME" bash "$script"
}
...
install_gitnexus_system() {
    ...
    _log "安装系统级 gitnexus@${SUPERNOVA_GITNEXUS_VERSION}（npm --prefix=/usr）"
    npm config set registry "$SUPERNOVA_NPM_REGISTRY" 2>/dev/null || true
    npm install -g --prefix=/usr --ignore-scripts "gitnexus@${SUPERNOVA_GITNEXUS_VERSION}"
...
fix_root_acl() {
    # 让 supernova-user 穿越 /root 进 /root/supernova（不动 others；比 777 安全）。
    if runuser -u "$SUPERNOVA_USER" -- test -x /root 2>/dev/null; then
        _skip "/root ACL（supernova-user 已可穿越）"; return; fi
    if command -v setfacl >/dev/null 2>&1; then
        _log "setfacl -m u:$SUPERNOVA_USER:x /root"
        setfacl -m "u:$SUPERNOVA_USER:x" /root || _die "setfacl 失败（装 acl 包或手动 chmod o+x /root）"
    else
...
fix_ownership() {
    # supernova-user 产物 + uv 元数据归 supernova-user；.env/.env.profiles 保持 root（密钥）。
    # ⚠️ 不 chown repos：repos 是 WEB(容器 root) 与 CLI(supernova-user) 共享的扫描目标，
    # chown 给任一方都让另一方 git dubious ownership（实测容器 root 访问 supernova-user
    # 属主 .git → exit 128，WEB 扫描挂）。repos 属主保持 clone 时的，dubious 靠
    # safe.directory 信任解决（supernova-user: ensure 设；容器 root: Dockerfile 设）。
    local d f
    for d in workspaces configs .venv; do
        if [ -e "$SUPERNOVA_HOME/$d" ]; then
            chown -R "$SUPERNOVA_USER:$SUPERNOVA_USER" "$SUPERNOVA_HOME/$d"
        fi
    done
    for f in uv.lock pyproject.toml; do
        [ -e "$SUPERNOVA_HOME/$f" ] && chown "$SUPERNOVA_USER:$SUPERNOVA_USER" "$SUPERNOVA_HOME/$f"
    done
    chown "$SUPERNOVA_USER:$SUPERNOVA_USER" "$SUPERNOVA_HOME"
    _log "归属 → $SUPERNOVA_USER（workspaces/configs/.venv + uv.lock/pyproject.toml；repos 不动）"
}

uv_sync_venv() {
    [ -d "$SUPERNOVA_HOME" ] || _die "$SUPERNOVA_HOME 不存在（先 git clone supernova 到此）"
    _log "uv sync（以 $SUPERNOVA_USER 身份，走 $SUPERNOVA_PYPI_INDEX）"
    runuser -u "$SUPERNOVA_USER" -- sh -c \
        "cd '$SUPERNOVA_HOME' && UV_DEFAULT_INDEX='$SUPERNOVA_PYPI_INDEX' uv sync"
}
...
verify() {
    ...
    _chk supernova-user    id "$SUPERNOVA_USER"
    _chk uv              test -x /usr/local/bin/uv
    _chk node            test -x /usr/bin/node
    _chk gitnexus        _has gitnexus
    _chk "root reachable" runuser -u "$SUPERNOVA_USER" -- test -x "$SUPERNOVA_HOME"
    _chk .venv           runuser -u "$SUPERNOVA_USER" -- test -d "$SUPERNOVA_HOME/.venv"
    # safe.directory：接受 repos/* 通配 或 全局 *
    local sd
    sd="$(runuser -u "$SUPERNOVA_USER" -- git config --global --get-all safe.directory 2>/dev/null || true)"
    if printf '%s\n' "$sd" | grep -Fxq "$SUPERNOVA_HOME/repos/*" \
        || printf '%s\n' "$sd" | grep -Fxq '*'; then
        _log "verify: safe.directory"
    else
        _fail "verify FAIL: safe.directory"; fails=$((fails+1))
    fi
    # 端到端：supernova-user 真能跑 uv run supernova-whitebox
    if runuser -u "$SUPERNOVA_USER" -- sh -c \
        "cd '$SUPERNOVA_HOME' && uv run --no-sync supernova-whitebox --help" >/dev/null 2>&1; then
        _log "verify: uv run supernova-whitebox --help"
    else
        _fail "verify FAIL: uv run supernova-whitebox --help"; fails=$((fails+1))
    fi
...
main() {
    ensure_root
    echo "=== supernova provision（$SUPERNOVA_HOME，用户 $SUPERNOVA_USER）==="
    install_docker
    ...
    uv_sync_venv
    verify
    echo ">> done：supernova-user 可跑 uv run supernova-whitebox；你可 bash scripts/up.sh 起 web"
}
