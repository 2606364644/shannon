...
def _find_bootstrap_script() -> Path | None:
    ...
    Resolution order:
      1. ``SUPERNOVA_BOOTSTRAP_SCRIPT`` env var (absolute path).
      2. Walk up from this file:
         runtime/ → supernova_core/ → src/ → core/ → packages/ → repo-root.
    """
    override = os.environ.get("SUPERNOVA_BOOTSTRAP_SCRIPT")
    if override:
        p = Path(override)
        return p if p.exists() else None

    # prerequisites.py lives at:
    #   packages/core/src/supernova_core/runtime/prerequisites.py
    # parents[5] = repo root (supernova/)
...
def ensure_prerequisite(name: str, *, profile: str) -> None:
    ...
    Environment variables:
        SUPERNOVA_SKIP_PREREQUISITES: Set to ``1`` to skip all checks (CI).
        SUPERNOVA_BOOTSTRAP_SCRIPT:  Override path to bootstrap.sh.
    """
    if os.environ.get("SUPERNOVA_SKIP_PREREQUISITES") == "1":
        logger.debug(
            "Skipping prerequisite check for %s (SUPERNOVA_SKIP_PREREQUISITES=1)",
            name,
        )
        return
...
def ensure_browser_engine(
    ...
    *,
    ...
    """
    import supernova_core.services.engines  # noqa: F401  (ensure engines registered)
