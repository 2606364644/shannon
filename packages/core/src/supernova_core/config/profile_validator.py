"""启动校验: 当前 profile 的变量与声明的 SUPERNOVA_AI_PROVIDER 是否自洽。
...
def validate_active_profile() -> None:
    ...
    if missing:
        raise PentestError(
            ...
            f"请在 .env.profiles/${{SUPERNOVA_PROFILE}}.env 补齐",
            category="config",
            error_code=ErrorCode.CONFIG_VALIDATION_FAILED,
