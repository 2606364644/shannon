...
# Resolve project root: this file is at packages/core/src/supernova_core/services/temporal_infra.py
_PROJECT_ROOT = Path(__file__).resolve().parents[5]

_CONTAINER_NAME = "supernova-temporal"
...
# WEB 路径用固定 queue，worker 容器常驻注册消费。前缀同（supernova-wb/bb）但值不同
...
async def get_temporal_status(
    ...
    source = "unknown"
    if healthy:
        for name in ["shannon-temporal", "supernova-temporal"]:
            try:
                result = subprocess.run(
...
async def ensure_infra(
    ...
    3. Otherwise, start supernova's own docker-compose as fallback.
    """
    ...
    if _shannon_container_exists():
        ...
        logger.info("No existing Temporal found — starting supernova container.")
