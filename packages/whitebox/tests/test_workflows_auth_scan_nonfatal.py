...
def test_auth_config_scan_call_is_non_fatal():
    ...
    # 调用前 200 字符内应有 try:；调用后到文件尾搜 warning 标注 non-fatal
    # （search-anywhere：无固定窗口，对未来日志增长免疫——final review Minor）
    before = src[max(0, idx - 200):idx]
    after = src[idx:]  # search-anywhere after the call — no fixed window (immune to future log growth)
