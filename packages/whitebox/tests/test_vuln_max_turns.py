...
vuln agent 用 SUPERNOVA_VULN_MAX_TURNS(默认 500);其他 agent 返回 None,
executor/run_claude_prompt 收到 None 时沿用全局 env 默认,行为零变更。
"""
import pytest

from supernova_whitebox.pipeline.activities import _vuln_max_turns


class TestVulnMaxTurns:
    def test_vuln_agent_returns_500_default(self, monkeypatch):
        monkeypatch.delenv("SUPERNOVA_VULN_MAX_TURNS", raising=False)
    ...
    def test_env_override(self, monkeypatch):
        monkeypatch.setenv("SUPERNOVA_VULN_MAX_TURNS", "800")
