...
调用, 导致错配 profile 静默放行。被测单元是 shannon_whitebox.cli.main.cli 的 wiring,
...
def test_misconfigured_profile_aborts_cli_startup(tmp_path, monkeypatch):
    ...
    profile 'bad' 声明 SHANNON_AI_PROVIDER=anthropic_api, 但缺必填的 ANTHROPIC_BASE_URL,
    profile_validator 应在 cli() group 回调里启动即失败。
    """
    # 清掉可能从宿主进程继承的 profile 变量, 确保 wiring 只读到 tmp_path 下的文件。
    for var in ("SHANNON_PROFILE", "SHANNON_AI_PROVIDER", "ANTHROPIC_BASE_URL"):
        monkeypatch.delenv(var, raising=False)

    _write(tmp_path / ".env", "SHANNON_PROFILE=bad\n")
    _write(
