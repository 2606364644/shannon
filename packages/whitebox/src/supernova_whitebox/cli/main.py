...
from supernova_core.config.concurrency import get_max_concurrent, is_llm_track_enabled
from supernova_core.config.vuln_selection import resolve_vuln_classes, InvalidVulnClass
...
from supernova_core.utils.paths import resolve_workspaces_dir, resolve_track_deliverable, WHITEBOX_SUBDIR
from supernova_whitebox.pipeline.shared import PipelineInput


@click.group()
def cli():
    """Supernova White-Box Scanner - Source code vulnerability analysis."""
    load_env()
...
@click.option(
    ...
    help="逗号分隔的 vuln 类（如 injection,xss）；优先于 SUPERNOVA_VULN_CLASSES env 与 YAML vuln_classes。"
)
def start(repo, output, workspace, config_path, pipeline_testing, temporal_address, plain, url, fresh, rewind, debug, vuln_classes_cli):
    ...
    try:
        override = resolve_vuln_classes(
            vuln_classes_cli,
            os.environ.get("SUPERNOVA_VULN_CLASSES"),
        )
    ...
    if result.get("status") == "cancelled":
        ...
        if deliverables_path:
            from supernova_core.workspace import summarize_deliverables_dir
        ...
        click.echo(f"    supernova-blackbox start --url {web_url} --repo {str(Path(repo).resolve())} -w {ws_name}")
        click.echo("    # or use --latest to reuse the most recent white-box results:")
        click.echo(f"    supernova-blackbox start --url {web_url} --repo {str(Path(repo).resolve())} --latest")
    else:
...
def logs(workspace_name, follow, diagnostic, full):
    ...
    if full:
        ...
        if not events_file.exists():
            ...
            return
        if follow:
            from supernova_core.cli.logs import tail_events_ndjson
